window.paypal
  .Buttons({
    // create order to initialise payment
    async createOrder() {
      try {
    ...
    // on payment approve/decline, return proper response
    async onApprove(data, actions) {
      try {
        ...
        if (errorDetail?.issue === "INSTRUMENT_DECLINED") {
          ...
          // (2) Other non-recoverable errors -> Show a failure message
          console.log('Capture error', errorDetail);
    ...
    async onCancel(data) {
      ...
      resultMessage(`Order canceled.<br><br>`);
    },

    async onError(data) {
      console.log("OnError", data);
      resultMessage(`An error occurred.<br><br>`);
    }
  })
